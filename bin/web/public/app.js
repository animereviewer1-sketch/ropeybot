...
function updateStatus(status) {
    document.getElementById("botStatus").textContent = status.connected
        ? "✅ Connected"
        : "❌ Disconnected";
    document.getElementById("gameName").textContent = status.game || "None";
    document.getElementById("playerCount").textContent =
        `${status.playerCount}/${status.roomLimit}`;
    ...
    document.getElementById("roomDescription").textContent =
        status.roomDescription || "No description";
    document.getElementById("roomPrivacy").textContent = status.roomPrivate
        ? "🔒 Private"
        : "🌐 Public";
}
...
async function fetchConfig() {
    ...
    if (config) {
        document.getElementById("superusers").value =
            config.superusers.join(", ");
        document.getElementById("members").value = config.members.join(", ");
        document.getElementById("configRoomName").value =
            config.room.Name || "";
        document.getElementById("configRoomDescription").value =
            config.room.Description || "";
        document.getElementById("configRoomPrivate").checked =
            config.room.Private || false;
        document.getElementById("configRoomLimit").value =
            config.room.Limit || 10;
    }
}
...
async function fetchBanList() {
    ...
    if (banList) {
        ...
        if (banList.length === 0) {
            container.innerHTML =
                '<div style="color: #888;">No banned players</div>';
        } else {
...
async function saveConfig() {
    const superusers = document
        .getElementById("superusers")
        .value.split(",")
